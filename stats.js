...
async function handleStatsUpdate(baseFolder) {
  ...
  try {
    ...
    const [leagueData, playerStats, goalieStats, scheduleData] = await Promise.all([
    ...
    // Build activeDivs from schedule. By default we only run STATS for
    // divisions that have games in the current week. If the
    // "allDivisionsCheckbox" is checked in the UI, we instead run STATS
    ...
    //run script for each active division
    for (let d=0; d<activeDivs.length; d++){
        ...
        for (let i=0; i<playerStats.length; i++){
            if (playerStats[i].div === confDiv){
                divPlayerStats.push(playerStats[i])
            }
        }    
        ...
        for (let i=0; i<goalieStats.length; i++){
            if (goalieStats[i].div === confDiv){
                divGoalieStats.push(goalieStats[i])
            }
        }
        ...
        statusEl.innerHTML = `Updating ${divAbb}...`;
        ...
        await core.executeAsModal(async () => {
            ...
            const exportFile = await prepareStatsExport(gamedayFolder, week, divAbb);
...
async function prepareStatsExport(gamedayFolder, week, divAbb){
